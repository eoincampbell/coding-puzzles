...
namespace AdventOfCode2019.Puzzles.Day01
{
    ...
    public class Impl: Puzzle<int, int>
    {
        ...
        public override async Task<int> RunPart1Async() => await Task.Run(() => Inputs.Select(CalcFuel).Sum());

        public override async Task<int> RunPart2Async() => await Task.Run(() => Inputs.Select(CalcFuelForFuel).Sum());

        private static int CalcFuel(int mass) => (mass / 3) - 2;

        private static int CalcFuelForFuel(int mass)  => CalcFuel(mass) > 0 
