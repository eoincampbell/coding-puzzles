...
namespace AdventOfCode2019.Puzzles.Day07
{
    ...
    public class Impl : Puzzle<string, BigInteger>
    {
        ...
        public override async Task<BigInteger> RunPart1Async() => await Task.Run(() =>
        {
            ...
            foreach (var phaseSettings in c)
            {
                ...
                if (nextInput > bestOutput) bestOutput = nextInput;
            }

            return bestOutput;
        });

        public override async Task<BigInteger> RunPart2Async() => await Task.Run(() =>
        {
            ...
            foreach (var phaseSettings in c)
            {
                ...
                while (vms[0].State != VmState.Halted)
                    for (var i = 0; i < 5; i++)
                    {
                        vms[i].RunProgramUntilOutputAvailable();
                        var nextOutput = vms[i].GetOutput();

                        if (i == 4 && vms[4].State == VmState.Halted) result = nextOutput;
                        else
                        {
                ...
                if (result > bestOutput) bestOutput = result;
            }
            return bestOutput;
        });


        private IList<IntCodeVm> GetVms(IList<int> phaseSettings) => Enumerable.Range(0, 5)
            .Select(r =>
            {
                var vm = new IntCodeVm(Inputs[0]);
                vm.SetInput(phaseSettings[r]);
                if (r == 0) vm.SetInput(0);
                return vm;
            }).ToList();
    }
}